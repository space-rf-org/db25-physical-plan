...
bool collect_join_region(const plan::LogicalNode& n, JoinRegion& out) {
    ...
    // Fewer than three leaves has nothing to reorder - the two-way case is what
    // build-side selection already covers. More than 64 exceeds the leaf-set
    // bitmask, and is far past any search budget worth running.
    if (out.leaves.size() < 3 || out.leaves.size() > 64) return false;
    out.output = &n.output;
    return true;
}

std::vector<std::uint32_t> subset_slots(const JoinRegion& region, std::uint64_t subset) {
    std::vector<std::uint32_t> slots(region.width(), kNoSlot);
    std::uint32_t next = 0;
    // Leaf order, not subset-membership order: a subset's canonical column order is
    // the order its leaves appear in the REGION. That is what makes the full set's
    // canonical order identical to the region's original one, so nothing above the
    // region has to change however the inside is rearranged.
    for (std::size_t i = 0; i < region.leaves.size(); ++i) {
        if ((subset & (std::uint64_t{1} << i)) == 0) continue;
        for (std::uint32_t c = 0; c < region.leaf_width[i]; ++c) {
            slots[region.leaf_offset[i] + c] = next++;
        }
    }
    return slots;
}
...
std::uint32_t subset_width(const JoinRegion& region, std::uint64_t subset) {
    std::uint32_t w = 0;
    for (std::size_t i = 0; i < region.leaves.size(); ++i) {
        if ((subset & (std::uint64_t{1} << i)) != 0) w += region.leaf_width[i];
    }
    return w;
}

plan::ExprPtr clone_translated(const plan::Expr& e, std::uint32_t base,
    ...
    out->input_index = e.kind == plan::ExprKind::ColumnRef ? slots[base + e.input_index]
                                                           : e.input_index;
    out->children.reserve(e.children.size());
    for (const auto& c : e.children) {
        out->children.push_back(clone_translated(*c, base, slots));
    }
    return out;
}
...
const plan::Expr* translate_conjunct(const RegionConjunct& c, const JoinRegion& region,
                                     ...
                                     LoweringArena& arena) {
    // The join's input schema is left's canonical columns followed by right's, so a
    // region column lands at its slot within left, or past left's width at its slot
    // within right.
    const std::vector<std::uint32_t> l = subset_slots(region, left);
    const std::vector<std::uint32_t> r = subset_slots(region, right);
    const std::uint32_t lw = subset_width(region, left);
    std::vector<std::uint32_t> slots(region.width(), kNoSlot);
    for (std::size_t i = 0; i < slots.size(); ++i) {
        if (l[i] != kNoSlot) slots[i] = l[i];
        else if (r[i] != kNoSlot) slots[i] = lw + r[i];
    }
    arena.exprs.push_back(clone_translated(*c.expr, c.base, slots));
